...
const Dashboard: React.FC = () => {
  ...
  const { getChartData, financialData, refreshStatistics, isLoading } =
    useStatistics();
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState<
    "all" | "revenue" | "received" | "sent"
  >("all");
            ...
            <CardHeader className="pb-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <Tabs
                  ...
                  onValueChange={(value) =>
                    setActiveTab(
                      value as "all" | "revenue" | "received" | "sent"
                    )
                  }
              ...
              <div className="h-64 sm:h-80 lg:h-96">
                ...
                {isLoading ? (
                  <div className="h-full flex items-center justify-center">
                    ...
                    (item) =>
                      item.revenue > 0 || item.received > 0 || item.sent > 0
                  ) ? (
                  ...
                  <div className="h-full flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="text-sm text-muted-foreground mb-4">
                        Your account activity chart will appear here once you
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3 min-w-0 flex-1">
                              <div
                                className={`p-2 rounded-full ${display.bgColor} flex-shrink-0`}
                              >
                ...
                <CardContent className="space-y-3 sm:space-y-4">
                  {cardsLoading ? (
                    ...
                    cards.slice(0, 5).map((card) => (
                      <div
                        key={card.id}
                        className={`relative overflow-hidden rounded-lg p-4 border hover:bg-muted/50 transition-colors ${
                          card.isDefault ? "ring-2 ring-primary" : ""
                        }`}
                      >
                        <div
                          className={`absolute top-0 left-0 h-full w-2 ${card.cardColor}`}
                        ></div>
